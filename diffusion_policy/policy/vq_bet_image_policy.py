...
class VQBeTPolicy(BaseImagePolicy):
    def __init__(self,
                 shape_meta: dict,
                 algo_name: str,
                 config: dict,
        ...
        self.obs_key_shapes = {key: list(attr['shape']) for key, attr in obs_shape_meta.items()}
        obs_config = {
            'low_dim': [],
            'rgb': [],
            'depth': [],
        ...
        self.vqbet = VQBeTModel(self.config)

        self.expected_image_keys = [k for k in self.config.input_shapes if k.startswith("observation.image")]

        self.reset()

    def _get_default_config(self):
        # Implement default configuration here
        pass
    ...
    def reset(self):
        ...
        # self.vqbet.reset()  # Assuming VQBeTModel has a reset method

    def to(self, *args, **kwargs):
        device, dtype, non_blocking, convert_to_format = torch._C._nn._parse_to(*args, **kwargs)
        if device is not None:
            self.vqbet = self.vqbet.to(device)
    ...
    def predict_action(self, obs_dict: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        if len(self._queues["action"]) == 0:
            batch = {k: torch.stack(list(self._queues[k]), dim=1) for k in nobs_dict if k in self._queues}
            nactions = self.vqbet(batch, rollout=True)[:, : self.config.action_chunk_size]
            actions = self.normalizer['action'].unnormalize(nactions)
    ...
    def _populate_queues(self, queues, batch):
        for key, queue in queues.items():
            if key in batch:
                queue.append(batch[key])
        return queues

    def action_dist(self, obs_dict: Dict[str, torch.Tensor]):
        nobs_dict = self.normalizer(obs_dict)
        nobs_dict = dict_apply(nobs_dict, lambda x: x[:, 0, ...])
        nobs_dict = dict_apply(nobs_dict, lambda x: x.to(self.vqbet.device))
        nobs_dict['observation.image'] = nobs_dict['image']
        nobs_dict["observation.images"] = torch.stack([nobs_dict[k] for k in self.expected_image_keys], dim=-4)
        return self.vqbet.get_action_dist(nobs_dict)  # Implement this method in VQBeTModel if needed
    ...
    def compute_loss(self, batch, epoch=None, validate=False):
        ...
        batch = {'observation.images': nobs['observation.images'], 'observation.state': nobs['observation.state'],
                 'action': nactions}
    ...
    def get_optimizer(self):
        return self.vqbet.get_optimizer()  # Implement this method in VQBeTModel
