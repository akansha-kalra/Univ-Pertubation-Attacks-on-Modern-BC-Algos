...
class TrainUniPertVQBeTImageWorkspace(BaseWorkspace):
    ...
    def run(self):
        ...
        if cfg.log:
            ...
            logging_cfg["tags"] = [str(tag) for tag in logging_cfg.get("tags", [])
                                   ]
            ...
            wandb_run=wandb.init(config=OmegaConf.to_container(cfg, resolve=True),  # epsilon stays float here
                       **logging_cfg)
            # wandb.config.update(
            #     {
            #         "output_dir": self.output_dir,
        ...
        with JsonLogger(log_path) as json_logger:
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if cfg.view == 'both':
                    total_grad = {}
                    for view in views:
                        total_grad[view] = torch.zeros((1, 1, image_shape[0], image_shape[1], image_shape[2])).to(
                            device)
                else:
                    total_grad = {}
                    ...
                    for batch_idx, batch in enumerate(tepoch):
                        ...
                        if cfg.log:
                            if cfg.view == 'both':
                                for view in views:
                                    wandb.log(
                                        {"gradient_magnitude": torch.norm(batch_cp['observation.images'].grad).item()})
                            else:
                                wandb.log(
                                    {"gradient_magnitude": torch.norm(batch_cp['observation.images'].grad).item()})
